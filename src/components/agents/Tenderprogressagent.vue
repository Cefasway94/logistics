                ...
                <v-flex sm12 md8 lg8 xlg8 >
                    <v-card width="" class="pt-6 pb-3 pl-8">
                        ...
                        <v-flex row class="mt-7 mb-4" >
                            ...
                            <v-flex column class="pl-3" v-if="documents == true && photo_url !== ''">
                            <p class="primary--text body-1 mb-2"> BILL OF LADING </p>
                ...
                <v-flex sm12 md3 lg3 xlg3 class="px-3 pt-2 ml-2" v-if="documents == true">
                    <v-card color="#4169E1" width="" class="py-4 px-5">
                        <v-flex row >
                            ...
                            <v-flex class="pl-8">
                            <v-flex v-if="LOAD_CUSTOMER.objects.mobile_number !== '' 
                                    && LOAD_CUSTOMER.objects.mobile_number !== null ">
                                <p  class="white--text mb-1">
                                    {{LOAD_CUSTOMER.objects.mobile_number}}
                                </p>
                            </v-flex>
                            <v-flex v-if="LOAD_CUSTOMER.objects.office_mobile !== '' 
...
export default {
    ...
    next(vm =>{  vm.GET_TENDERSDETAILs(to.params.id).then(()=>{
          ...
          vm.GET_AGENT(localStorage.client).then(()=>{
              ...
              let tendertype = 'Clearing'

            vm.GET_PAYMENT_PROGRESS( {payload: to.params.id, tendertype: tendertype}).then(()=>{
                ...
                if (vm.LOAD_PAYMENT_PROGRESS.objects.length === 0 && 
                    ...
                    vm.GET_OXOPAYMENT_PROGRESS({payload: to.params.id, tendertype: tendertype}).then(()=>{
       ...
       getFileExtension(url){
        ...
        //        const   expected_date = this.date
           ...
           this.$store.dispatch('UPGRADE_PROGRESS',{
                    ...
                    this.GET_PROGRESS_STAGES(this.$route.params.id).then(()=>{
